...
This is the only property of the ToolResult that is always required, as it is the only one that the agent uses to understand the history of interaction events. Meaning, if you don't return anything in the data property, the agent will not be informed about your result, and it will not be able to it to navigate the interaction.
...
#### Canned Responses
Tools can also return complete canned responses for consideration, as well as fields to be substituted during canned response rendering. For more information about canned response properties, refer to the  section.
...
Direct command/query activation in-context
...
### Agentic API Design
To learn more about best-practices for designing your agent's tools, we recommend reading our blog post on .
...
- `agent_id`: The unique identifier of the agent that is calling the tool.
- `customer_id`: The unique identifier of the customer interacting with the agent.
- `session_id`: The unique identifier of the current session.
- `emit_message(message: str)`: A method to send a message back to the customer. This can be used to report progress during a long-running tool call.
- `emit_status(status: p.SessionStatus)`: A method to update the .
...
A better and more reliable way to do this is to  with Parlant and use the information available programmatically, which is contained in the ToolContext parameter of your tool.
